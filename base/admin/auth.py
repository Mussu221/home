...
from base.admin.models import Admin , Terms , Privacy 
...
def register():
    if request.method=='POST':
        ...
        if admin :
            ...
            admin = Admin(fname=fname,lname=lname,email=email,password=hashed_password,image_file="default.png",created_at=datetime.utcnow())
            insert_data(admin)
            login_user(admin)
...
def save_picture(form_picture):
    if form_picture.filename !="default.png":
...
def terms():
    ...
    if  current_user.is_anonymous:
        ...
        if content:
            ...
            term = Terms(content=request.form.get("body"),created_at=datetime.utcnow())
            insert_data(term)
            save()
...
def privacy_policy():
    ...
    if  current_user.is_anonymous:
        ...
        if content:
            ...
            term = Privacy(content=request.form.get("body"),created_at=datetime.utcnow())
            insert_data(term)
            save()
...
@admin_auth.route("/terms1", methods=['GET', 'POST'])
def terms1():
